...
const Article = ({ img, title, date, link }) => {
  return (
    ...
    bg-light text-dark first:mt-0 border border-solid border-dark border-r-4 
    border-b-4 dark:border-light dark:bg-dark dark:text-light
    sm:flex-col">
...
const FeaturedArticle = ({ img, time, title, summary, link }) => {
  return (
    <li className='relative col-span-1 w-full p-4 bg-light border border-solid border-dark rounded-2xl dark:bg-dark dark:border-light'>
      <div className='absolute top-0 right-0 z-[-10] w-[101%] h-[103%] rounded-[2rem] bg-dark rounded-br-3xl' />
...
const Blog = () => {
  return (
    <>
      <Head>
        <title>My react starter | Company Blog</title>
        <meta name="description" content="For Visionary Contents Creators." />
      </Head>
      <TransitionEffect />
      <main className='w-full mb-16 flex flex-col items-center justify-center overflow-hidden dark:text-light'>
        <Layout className='pt-16'>
          <AnimatedText text="Latest Articles" className='mb-16 
          ...
          <ul>
            ...
            <Article
              ...
              img={article7}
            />
            {/* Repeat Article component for other articles */}
          </ul>
        </Layout>
      </main>
    </>
